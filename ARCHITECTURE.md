...
- **Written against a backend that didn't exist yet at Step 10** — `GET /api/admin/roles` and `PUT /api/admin/roles/:systemRole/permissions` weren't implemented in `admin.controller.ts`/`admin.routes.ts` at the time. Marked `// BACKEND GAP` in `useAdmin.ts`, the same convention Step 4's frontend used for the gaps Steps 7–8 later closed. Not part of Step 10's requested scope (frontend-only), but flagged here rather than silently shipping a dead mutation.

Both `hims-frontend` (`tsc -b`, `vite build`) verify clean.

### Post-Step-16 fix — closing the Step 10 Role/Permission gap

`admin.service.ts#listRoles`/`updateRolePermissions`, `admin.controller.ts#listRolesAdmin`/`updateRolePermissionsAdmin`, and the matching `GET /api/admin/roles` / `PUT /api/admin/roles/:systemRole/permissions` routes now exist. `listRoles` auto-seeds a `Role` document (empty permissions, `isEditable: role !== SUPER_ADMIN`) for any `SystemRole` that doesn't have one yet — there's no separate DB seed script, so this is what makes `RoleManagement.tsx` work out of the box on a brand-new database. `updateRolePermissions` rejects edits to a non-editable role (`ForbiddenError`) and calls `rbac.middleware.ts#invalidatePermissionCache` after saving, so a permission change is enforced by `authorizePermission` immediately rather than waiting out its 5-minute Redis cache TTL. The stale `// BACKEND GAP` comments this closed (`useAdmin.ts`, plus four already-inaccurate ones in `auth.types.ts`/`axios.ts`/`LoginPage.tsx`/`AuthContext.tsx` left over from before Step 7 actually shipped auth) have been removed — `grep -rn "BACKEND GAP" hims-frontend/src` now returns nothing, for real.
...
IVF/Obstetric/Pediatric/DMO Specialty EMRs, and the Medical Record Department (MRD) —
...
- **Pediatric EMR** (`PediatricRecord.model.ts`, added post-Step-14 to close a gap in the original request — the heading named Pediatric alongside IVF/Obstetric/DMO but no body bullet had specified it yet) — one record per child, the same growing-document shape as `IvfCycle`/`ObstetricRecord`, holding two embedded time series: `vaccinationSchedule` (seeded at record creation from `STANDARD_IMMUNIZATION_SCHEDULE`, computing each dose's real due date off the patient's own `dateOfBirth` rather than leaving the ward to hand-enter every dose) and `growthChartEntries` (weight/height/head-circumference, each snapshotting `ageInMonths` at recording time so the chart's x-axis never shifts). `recordVaccineAdministered` stamps the batch number and administering user per dose; `skipDose` covers a medically-contraindicated dose without pretending it was given. Frontend's `GrowthChart.tsx` plots each of the three metrics as its own small trend line (they live on incompatible scales) and deliberately doesn't overlay WHO's own percentile bands — the same "mimic the shape, not the full standard" scoping the DICOM Modality Worklist used in Step 13 — while `PediatricRecordDetailModal.tsx` lists every dose with its due/administered state and an inline administer-or-skip action.
...
- [x] **Step 10** — Frontend optimization (Vite `manualChunks` code-splitting, resolving the bundle-size warning) and the RBAC permission-matrix editor (`RoleManagement.tsx`). Originally built against `GET/PUT /api/admin/roles*` endpoints that didn't exist yet; closed in the post-Step-16 fix documented above — the backend now auto-seeds a `Role` per `SystemRole` and both routes are live.
